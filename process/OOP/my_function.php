...
namespace App;
    ...
    class RouterGet {
        ...
        public function handleRequest(string $url): void {
            $query = parse_url($url, PHP_URL_QUERY); // Получаем строку запроса из URL
            if ($query) {
                parse_str($query, $params); // Преобразуем строку запроса в ассоциативный массив
                ...
                // Если запрос без строки запроса, вызываем обработчик для корневого адреса
                $this->handleRoot();
                return;
            }
            $this->handleNotFound();
        ...
        private function handleNotFound(): void {
            echo "<section class='base' id='what?'><p>Как вы сюда попали ?</p><p>В любом случае, 404 Not Found</p</section>";
        }
    }
    ...
    class TemplateEngine //Шаблонизатор-[[переменная]] как в MODX
    {
        private $data;
        private $db;
        ...
        public function renderTemplate($html_sql) {
            $pattern = '/\[\[(.*?)\]\]/'; 
        ...
        public function getPage($id) {
            $page = $this->db->prepare("SELECT * FROM hablons WHERE id_page = :id");
            $page->execute(['id' => $id]);

            while ($page_data = $page->fetch(PDO::FETCH_ASSOC)) {
                $html_sql = $page_data['page_text'];

                require_once 'config/snipets.php';

                return $this->renderTemplate($html_sql);
            }
        }
    }
namespace Session;
